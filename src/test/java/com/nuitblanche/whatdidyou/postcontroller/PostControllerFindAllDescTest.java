...
public class PostControllerFindAllDescTest {
    ...
    public void findPostById(){
        ...
        ResponseEntity<SingleResult> postResponseEntityTwo = restTemplate.exchange(postUrl, HttpMethod.POST,postRequestEntity,SingleResult.class);
        assertThat(postResponseEntityOne.getStatusCode()).isEqualTo(HttpStatus.OK);

        String getUrl = "http://localhost:" + port + "/api/v1/posts";
        ...
        List<Object> responseList = mapper.convertValue(getResponseEntity.getBody().getList(),List.class);
        for(Object o : responseList){
            responseDtos.add(mapper.convertValue(o,PostResponseDto.class));
        }

        assertThat(responseDtos.size()).isEqualTo(2);
        assertThat(responseDtos.get(0).getId()).isEqualTo(2L);
        assertThat(responseDtos.get(0).getTitle()).isEqualTo(title);
        assertThat(responseDtos.get(0).getContent()).isEqualTo(content);
        assertThat(responseDtos.get(0).getPureText()).isEqualTo(pureText);
        assertThat(responseDtos.get(0).getTagTypes().size()).isEqualTo(3);

        assertThat(responseDtos.get(1).getId()).isEqualTo(1L);
        assertThat(responseDtos.get(1).getTitle()).isEqualTo(title);
        assertThat(responseDtos.get(1).getContent()).isEqualTo(content);
        assertThat(responseDtos.get(1).getPureText()).isEqualTo(pureText);
        assertThat(responseDtos.get(1).getTagTypes().size()).isEqualTo(3);
    }
}
