...
public class ProjectControllerUpdateTest {
    ...
    public void updateProject(){
        ...
        ProjectUpdateRequestDto updateRequestDto = ProjectUpdateRequestDto.builder()
                .title(updatedTitle)
                .build();

        String getUrl = "http://localhost:" + port + "/api/v1/projects/" + project.getId();

        HttpHeaders updateHeaders = new HttpHeaders();
        updateHeaders.setContentType(MediaType.APPLICATION_JSON);
        updateHeaders.set("Authorization","Bearer "+token);
        HttpEntity<ProjectUpdateRequestDto> updateRequestEntity = new HttpEntity<>(updateRequestDto,updateHeaders);

        ResponseEntity<SingleResult> updateResponseEntity = restTemplate.exchange(getUrl, HttpMethod.PUT,updateRequestEntity,SingleResult.class);
